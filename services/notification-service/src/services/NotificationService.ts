...
export class NotificationService {
  ...
  private async rateLimitSms(userId: string): Promise<void> {
    ...
    if (!countStr) {
      await redisClient.set(key, '1', 3600);
    } else {
  ...
  async sendSingle(payload: {
    channel: Channel,
    recipient: { email?: string; phone?: string; pushToken?: string; id?: string },
    templateId?: string,
    subject?: string,
    content?: string,
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
      try {
        if (payload.channel === 'EMAIL') {
          ...
          if (!email || !this.email.isValidEmail(email)) throw new Error('Invalid email');
          ...
          if (!phone || !this.sms.isValidPhoneVN(phone)) throw new Error('Invalid phone');
          await this.rateLimitSms(payload.recipient.id || phone);
          const parts = this.sms.splitMessage(content);
          for (const part of parts) await this.sms.send(phone, part);
        ...
        logger.error('Send attempt failed', { attempt, err: err?.message });
        if (attempt < 2) await this.backoff(attempt);
        else {
  ...
  async sendBatch(items: Array<Parameters<NotificationService['sendSingle']>[0]>) {
    const results = [] as any[];
    for (const item of items) {
      try {
        const r = await this.sendSingle(item);
        results.push({ ok: true, id: (r as any).id });
      } catch (e: any) {
